...
def _canonical(lock_path: Path) -> str:
    ...
    # Résolution INCONDITIONNELLE non-stricte du parent : deux StoreLock du même lockfile —
    # construits avant vs après création du data_dir, ou via un symlink (p.ex. macOS /var→/private/var) —
    # partagent la MÊME entrée de registre (invariant §5 niveau-1 : RLock partagé par chemin canonique).
