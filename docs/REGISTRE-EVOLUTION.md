...
| RS-056 | **Provisionnement du jeton d'auth cible non-autonome** — `claude setup-token` exige un **abonnement + flux interactif** (aucun flag non-interactif) ; `CLAUDE_CODE_OAUTH_TOKEN`/`ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN` **absents** de l'env. Le dispositif cible (HOME isolé + jeton) **ne peut être provisionné headless** → **preuve d'étanchéité cible DIFFÉRÉE** (option B propriétaire). B1 reste défaut (RS-4). | 2026-08-21 | J3 T1 (D5/D6, obstacle démontré) | `consignée` | **Acte propriétaire** : provisionner le jeton (`claude setup-token` interactif **ou** `ANTHROPIC_API_KEY` hors dépôt), puis rejouer B1 + cible ; ancré RS-030 |
